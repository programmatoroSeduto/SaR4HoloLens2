...
COMMENT 
    ...
    IS 'the token is used for inheriting sessions from previously defined ones';
COMMENT 
...
COMMENT 
    ON COLUMN sar.F_HL2_STAGING_WAYPOINTS.CURRENT_WAYPOINT_FK 
    IS '(self key) each time HoloLens2 sends a export ad indicates its current position as CurrentZone waypoint. The current zone wp can be found doing SELECT for primary key = CURRENT_WAYPOINT_FK given one measurement';
COMMENT 
